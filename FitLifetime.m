...
function [varargout] = FitLifetime(priorfit,model,histogram,PLOT)
...
f=3;   % # of fitted parameters
t=model.width*(0:model.num_chan_exp-1);
idealFit=conv(exp(-t/tau),model.IRF);  % ideal measurement
idealFit=circshift(idealFit,20*(0.128/model.width),2);  % in case you want 4ps resolution
...
%         t_ToBeFit=model.width*((1:length(varargout{4}))+(maxFitted-maxToBeFit));
        t=model.width*(1:length(varargout{4}));
        figure;
        plot(t,S*Fit); hold on;
        scatter(t,hist); 
